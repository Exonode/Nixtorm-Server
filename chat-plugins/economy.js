...
let shop = [
		...
		["Fix", "Buys the ability to alter your current Custom Avatar or Room Name. (Don\'t buy if you have neither)", 10],
		["League Room", "Purchases a room at a reduced rate for use with a league.", 35],
		["Avatar", "Buys an custom avatar to be applied to your name (You supply. Images larger than 80x80 may not show correctly)", 25],
		["Help", "A staff member will set up your rooms intro, website and anything else that is required.", 55],
		["Room", "Buys a chatroom for you to own. (within reason, can be refused)", 85],
		["Icon", "Buys a userlist icon that can be applied to the userlist of 3 rooms.", 115],
		["Protect", "Protects your room from being deleted automatically.", 150],
];
...
exports.commands = {
	...
	shop: function(target, room, user) {
		...
		if (room.id === '' && this.broadcasting) {
			...
			var buttonStyle = '';
			var topStyle = 'background: linear-gradient(grey); color: grey; border: 1px solid black; padding: 2px; border-radius: 5px;';
			var descStyle = 'border-radius: 5px; border: 1px solid #AAAAAA; background: #ccccff; color: black;';
			var top = '<td><center><b><p><h1><img src="http://media.pldh.net/pokemon/conquest/sprite/144.png" height=100 width=100>Nixtorm Shop<img src="http://media.pldh.net/pokemon/conquest/sprite/144.png" height=100 width=100></b></p></h1><table style="' + topStyle + '" border="5" cellspacing ="5" cellpadding="5"><tr><th>Item</th><th>Description</th><th>Cost</th></tr>';
			var bottom = '<table><td style="' + descStyle + '">To buy an item from the shop, use /buy command. If you want to know how to get money, check the Room Intro.</td>';
			...
			return this.sendReply('|raw|' +
				...
				table("Fix", "Buys the ability to alter your current Custom Avatar or Room Name. (Don\'t buy if you have neither)", 10) +
				table("Avatar", "Buys an custom avatar to be applied to your name (You supply. Images larger than 80x80 may not show correctly)", 25) +
				table("League Room", "Purchases a room at a reduced rate for use with a league.", 35) +
				table("Help", "A staff member will set up your rooms intro, website and anything else that is required.", 55) +
				table("Room", "Buys a chatroom for you to own. (Within reason, can be refused)", 85) +
				table("Icon", "Buys a userlist icon that can be applied to the userlist of 3 rooms.", 115) +
				table("Protect", "Protects your room from being deleted automatically.", 150) +
				bottom
			);
		}
	},
