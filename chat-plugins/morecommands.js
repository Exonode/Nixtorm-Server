...
exports.commands = {
	...
	auth: 'nal',
	nixtormauthlist: 'nal',
	nal: function(target, room, user, connection) {
		fs.readFile('config/usergroups.csv', 'utf8', function(err, data) {
			...
			connection.popup('|html|' +
				'<h3><center><u>Nixtorm Authority List</u></center></h3>' +
				'<b><u><i>~Administrators (' + staff['admins'].length + ')</u></b>:<br />' + staff['admins'].join(', ') +
				'<br /><b><u><i>&Leaders (' + staff['leaders'].length + ')</u></b>:<br />' + staff['leaders'].join(', ') +
	...
	credits: function (target, room, user) {
		function name (name, bold) {
			if (bold) {
				return "<b><font color=" + hashColor(name) + ">" + Tools.escapeHTML(name) + "</font></b>";
			} else {
				return "<font color=" + hashColor(name) + ">" + Tools.escapeHTML(name) + "</font>";
			}
		}
		var popup = "|html|" + "<font size=5><center><b>Nixtorm Server Credits</b></center></font><br />" +
					"<u>Owners:</u><br />" +
					"- " + name('Ransei', true) + " (Owner, Lead Policy, Sysadmin)<br />" +
					"- " + name('Chirou', true) + " (Server Host, Development, Sysadmin)<br />" +
					"- " + name('The Sakurai', true) + " (Founder)<br />" +
					"- " + name('Carnie Moe Sedah', true) + " (League Owner, Sysadmin)<br />" +
					"<br />" +
					"<u>Contributors:</u><br />" +
					"- " + name('Carnie Moe Sedah', true) + " (League Owner, Sysadmin)<br />" +
					"- " + name('Chirou', true) + " (Server Host)<br />" +
					"- " + name('fender', true) + " (Technical Advisor)<br />" +
					"<br />" +
	...
	removebadge: function(target, room, user) {
		...
		var self = this;
		var type_of_badges = ['admin', 'bot', 'dev', 'vip', 'artist', 'mod', 'leader', 'champ', 'creator', 'comcun', 'twinner', 'goodra', 'league', 'fns'];
	...
	givebadge: function(target, room, user) {
		...
		var self = this;
		var type_of_badges = ['admin', 'bot', 'dev', 'vip', 'mod', 'artist', 'leader', 'champ', 'creator', 'comcun', 'twinner', 'league', 'fns'];
	...
	badgelist: function(target, room, user) {
		if (!this.canBroadcast()) return;
		var fns = '<img src="http://www.smogon.com/media/forums/images/badges/forummod_alum.png" title="Former Nixtorm Staff">';
		var admin = '<img src="http://www.smogon.com/media/forums/images/badges/sop.png" title="Server Administrator">';
		var dev = '<img src="http://www.smogon.com/media/forums/images/badges/factory_foreman.png" title="Nixtorm Developer">';
		...
		return this.sendReplyBox('<b>List of Gold Badges</b>:<br>' + fns + '  ' + admin + '    ' + dev + '  ' + creator + '   ' + comcun + '    ' + mod + '    ' + leader + '    ' + league + '    ' + champ + '    ' + artist + '    ' + twinner + '    ' + vip + '    ' + bot + ' <br>--Hover over them to see the meaning of each.<br>--Get a badge and get a FREE custom avatar!<br>--Click <a href="http://goldserver.weebly.com/badges.html">here</a> to find out more about how to get a badge.');
	},
	badges: 'badge',
	badge: function(target, room, user) {
		...
		fs.readFile('badges.txt', 'utf8', function(err, data) {
			...
			if (match == true) {
				var badgelist = '';
				if (currentbadges.indexOf('fns') > -1) badgelist += ' ' + fns;
...
Gold.pmAll = function(message, pmName) {
	pmName = (pmName ? pmName : '~Nixtorm Server [Do not reply]');
